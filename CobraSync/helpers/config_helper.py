...
import CobraSync.helpers.crypto_helper
import CobraSync.helpers.log_helper
import configobj
import os
...
def decrypt_passwords(section, key):
    ...
    crypter = CobraSync.helpers.crypto_helper.Crypter()
    if key == "password":
...
def get_general_configfile_from_envvar():
    ...
    '''
    config_directory = os.environ['COBRASYNCHOME']
...
def get_config():
    ...
    logger = CobraSync.helpers.log_helper.init_logging(config)
